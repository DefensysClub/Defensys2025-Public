...
Flag = "defensys{SSS_1s_4m4z1ng_but_fl1ppings_is_dangerous}"
...
while counter <= 300:
    ...
    if choice == "1":
        ...
        try:
            print("Enter your share in the format x,y")
            sys.stdout.flush()
            your_share = tuple(map(int,sys.stdin.readline().strip("").split(",")))
            if len(your_share) != 2:
                print("Invalid share")
                continue
        ...
        try:
            ...
            sys.stdout.flush()
            your_secret = int(sys.stdin.readline().strip())
